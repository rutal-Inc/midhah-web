...
export default function RenderLyricsListWithSr({ slug }: Readonly<Params>) {
  ...
  useEffect(() => {
    if (slug && hasMoreData) {
      setIsLoading(true);
      fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/lyrics/poet/${slug}?page=${page}&size=30`,
        {
          method: "GET",
          headers: {
        ...
        {lyrics.map((lyric: Lyrics, index: number) => {
          ...
          return (
            <Fragment key={lyric.slug + index}>
              {showLetter && (
                <li className="pl-1 mt-1 col-span-2 flex align-middle">
                  <h2 className="text-3xl  text-gray-800 ml-4">
                    {firstLetter}
                  </h2>
                </li>
              )}

          <Link href={`/${lyric.genre}/${lyric.slug}`} key={lyric.slug + index}>
            <li
